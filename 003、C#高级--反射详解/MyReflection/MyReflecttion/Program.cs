        ...
        static void Main(string[] args)
        {
            try
            {
...

                {
                    ////三、反射 + 工厂 + 配置文件
                    ////问题：反射创建对象代码很多。
                    ////1、其实除了dll的名称和类的全名称都是一样的代码，可以封装反射创建对象帮助类。
                    ////2、传入的参数都是字符串，考虑做成配置项，提高代码的扩展性和灵活性，可以做到不修改代码而改变程序的功能。

                    //IDBHelper helper1 = SimpleFactory.CreateInstance();
                    //helper1.Query();
                    ////如果要将SqlServer换成MySql
                    ////1、传统方式：必须要修改代码，然后要重新编译发布，步骤很多
...

                {
                    ...
                    //    Console.WriteLine("********************传统单例***************************");
                    //}
                    ////new Singleton();//私有构造函数不能外部访问，但是反射可以
                    //{
                    //    //2、反射方式创建单例类对象
                ...
                {
                    ////八、反射操作属性字段
                    ////1、普通方法调用属性字段简单快捷，反射操作麻烦点。
                    ////2、类增加一个字段呢，普通方法调用需要修改代码，重新编译发布，代码不稳定，反射赋值没啥优势，反射取值不需要修改代码，代码就更加稳定。
                    ////3、type.GetProperties()获取属性，type.GetFields()获取字段。
